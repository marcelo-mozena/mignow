...
// --- Fields for Veículo Modelo ---

const REQUIRED_FIELDS: Array<{ field: string; label: string }> = [
  { field: 'nome', label: 'Nome' },
  { field: 'fabricante_id', label: 'Fabricante ID' },
...
export class VeiculoModeloImportService implements IImportService {
  ...
  async importRecord(ctx: ImportContext, record: Record<string, unknown>): Promise<void> {
    ...
    const fabricanteId = String(record.fabricante_id).trim();
    ...
    await apiRequest(
      `${ctx.baseUrl}/client/tms-base/v2/veiculos/fabricante/${fabricanteId}/modelo`,
      {
        method: 'POST',
        headers,
